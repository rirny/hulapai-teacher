...
class CourseController extends Yaf_Controller_Base_Abstract {
	...
	public function indexAction(){
		...
		$courseList = $_Course->getList($page,20,array('teacher'=>$this->tid));
		if($courseList['data']){
	...
	public function addAction(){
		if($this->_POST){
			...
			if($typeInfo['name'] !="其他" && $_Course->getRow(array('teacher'=>$this->tid,'type'=>$type)))   show_message('分类已添加！');
			$data = array(
				'type'=>$type,
				'title'=>$title,
				'teacher'=>$this->tid,
	...
	public function editAction(){
		if($this->_POST){
			...
			$_Course = new CourseModel();
			$courseInfo = $_Course->getRow(array('teacher'=>$this->tid,'id'=>$id));
			...
			if(!$_Course->updateData($data,array('teacher'=>$this->tid,'id'=>$id)))    show_message('分类修改失败！');
	...
	public function deleteAction(){
		...
		show_message('删除成功！',url('teacher','course'));
	}
}