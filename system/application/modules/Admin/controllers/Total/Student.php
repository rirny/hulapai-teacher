 ...
 * 学生统计
 */
class Total_StudentController extends Yaf_Controller_Base_Abstract {
	public function indexAction(){
		$agent = $this->get('agent',-1,'intval');
		$where = array();
		if($agent >=0) $where['agent'] = $agent;
		$_Student = new StudentModel();
    	$total = $_Student->getAll($where,'count(1) as nums,agent','','agent');
	...
	public function dayAction(){
		...
		if($agent >=0) $where['agent'] = $agent;
		$_Student = new StudentModel();
		$total = $_Student->getAll($where,array("COUNT(1) AS nums,agent,FROM_UNIXTIME(create_time,'%Y-%m-%d') AS create_date"),'create_date desc','create_date,agent');
	...
	public function detailAction(){
		...
		if($agent >=0) $where['agent'] = $agent;
		$_Student = new StudentModel();
		$students = $_Student->getList($page,20,$where,'*','','create_time desc');
