 ...
 * 微博统计
 */
class Total_FeedController extends Yaf_Controller_Base_Abstract {
	public function indexAction(){
		$from = $this->get('from',-1,'intval');
		$where = array();
		if($from >=0) $where['from'] = $from;
		$_Feed = new FeedModel();
    	$total = $_Feed->getAll($where,'count(1) as nums,from','','from');
	...
	public function dayAction(){
		...
		if($from >=0) $where['from'] = $from;
		$_Feed = new FeedModel();
		$total = $_Feed->getAll($where,array("COUNT(1) AS nums,`from`,FROM_UNIXTIME(publish_time,'%Y-%m-%d') AS create_date"),'create_date desc','create_date,from');
	...
	public function detailAction(){
		...
		if($from >=0) $where['from'] = $from;
		$_Feed = new FeedModel();
		$feeds = $_Feed->getList($page,20,$where,'*','','publish_time desc');
		$this->getView()->assign('pages', $feeds['pages']);
		$this->getView()->assign('datas', $feeds['data']);
    	$this->getView()->assign('start_date',$start_date);
    	$this->getView()->assign('end_date',$end_date);
    	$this->getView()->assign('from',$from);
	}
